...
export function operatorCompliance(operator,now=new Date()){
  ...
  const workAuthorised=(operator.status||'invited')==='active'&&!blocking.length,capabilities={home:workAuthorised&&!operator.homeSuspended,remote:workAuthorised&&!operator.remoteSuspended};
  return{operatorId:operator.id,name:operator.name,territoryIds:operator.territoryIds||[],operatorStatus:operator.status||'invited',workAuthorised,capabilities,status:blocking.length?'blocked':warnings.length?'expiring':'current',blockingCount:blocking.length,warningCount:warnings.length,checks};
}
export function operatorCanReceiveWork(operator,now=new Date(),service=''){const lifecycleStarted=['trainingStatus','dbsStatus','photoStatus','agreementStatus','insuranceStatus'].some(field=>field in (operator||{})),authorised=lifecycleStarted?operatorCompliance(operator,now).workAuthorised:(operator?.status||'active')==='active';return authorised&&(!service||!operator?.[`${service}Suspended`])}
