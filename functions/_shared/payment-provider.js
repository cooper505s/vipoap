...
export async function createCheckout(env,payload,idempotencyKey){
  ...
  const embedded=Boolean(env.STRIPE_PUBLISHABLE_KEY),form=new URLSearchParams({mode:'payment',client_reference_id:payload.reference,customer_email:payload.customer.email,'line_items[0][quantity]':'1','line_items[0][price_data][currency]':'gbp','line_items[0][price_data][unit_amount]':String(payload.amount),'line_items[0][price_data][product_data][name]':payload.description,'payment_intent_data[receipt_email]':payload.customer.email});
  if(embedded){form.set('ui_mode','embedded');form.set('return_url',payload.successUrl)}else{form.set('success_url',payload.successUrl);form.set('cancel_url',payload.cancelUrl)}
  ...
  const data=await response.json().catch(()=>({}));if(!response.ok||!data.id||embedded&&!data.client_secret||!embedded&&!data.url)throw new Error(data.error?.message||'Unable to create Stripe Checkout.');return{id:data.id,checkoutUrl:data.url||'',clientSecret:data.client_secret||'',publishableKey:embedded?env.STRIPE_PUBLISHABLE_KEY:''};
}
