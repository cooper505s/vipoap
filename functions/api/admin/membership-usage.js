...
export async function onRequestPost({request,env}){
  ...
  if(!before?.active||!before.personCovered)return Response.json({error:'An active membership is required for the member remote-support rate.'},{status:409});
  const rules=await platformRules(env),includedMinutes=0,paidMinutes=minutes,billableBlocks=Math.ceil(minutes/before.memberRemoteMinutes),amount=Number((billableBlocks*before.memberRemotePrice).toFixed(2)),engineerAmount=Number((billableBlocks*Number(rules.membership.engineerRemote30||14)).toFixed(2)),now=new Date().toISOString(),key=`membership-usage:${customerId}:${crypto.randomUUID()}`;
  const usage={customerId,personId:clean(body.personId),bookingKey:clean(body.bookingKey),minutes,includedMinutes,paidMinutes,billableBlocks,ratePer30:before.memberRemotePrice,amount,engineerAmount,currency:'GBP',periodStart:before.period.start,periodEnd:before.period.end,status:'posted',createdAt:now};
