...
export async function onRequestPost({request,env}){
  ...
  const body=await request.json().catch(()=>({})),key=clean(body.bookingKey,240),booking=key.startsWith('booking:')?await env.VIPOAP_DATA.get(key,'json'):null;if(!booking||!completedBooking(booking)||!canAccessTerritory(context,booking.territoryId||'andover'))return Response.json({error:'Completed job not found.'},{status:404});
