...
export async function onRequestGet({request,env}){
  ...
  const [bookingKeys,calloutKeys]=await Promise.all([env.VIPOAP_DATA.list({prefix:'booking:'}),env.VIPOAP_DATA.list({prefix:'callout:'})]),bookings=await Promise.all(bookingKeys.keys.map(async key=>({key:key.name,...await env.VIPOAP_DATA.get(key.name,'json')}))),callouts=await Promise.all(calloutKeys.keys.map(async key=>({key:key.name,...await env.VIPOAP_DATA.get(key.name,'json')}))),work=[...bookings,...callouts.filter(x=>!x.linkedBookingKey||!bookings.some(b=>b.key===x.linkedBookingKey))],rules=await platformRules(env),records=work.filter(item=>(item.operatorId||item.assignedEngineerId)===operatorId&&canAccessTerritory(context,item.territoryId||'andover')&&completedBooking(item)).map(item=>{const amountPence=providerEntitlementPence(item,rules),paid=Boolean(item.providerPaidAt||item.providerPaymentStatus==='paid');return{key:item.key,reference:item.reference||'',date:item.completedAt?.slice(0,10)||item.date||'',service:item.service||item.category||'',supportType:item.supportType||'Home visit',duration:Number(item.duration||0)+(item.additionalTime||[]).reduce((sum,entry)=>sum+Number(entry.minutes||0),0),amountPence,status:paid?'paid':'awaiting payout',paidAt:item.providerPaidAt||'',paymentReference:item.providerPaymentReference||'',stripeTransferId:item.providerStripeTransferId||''}}).sort((a,b)=>String(b.date).localeCompare(String(a.date)));
...
export async function onRequestPatch({request,env}){
  ...
  const body=await request.json(),key=clean(body.key,240),booking=/^(booking|callout):/.test(key)?await env.VIPOAP_DATA.get(key,'json'):null;if(!booking||!canAccessTerritory(context,booking.territoryId||'andover')||!completedBooking(booking))return Response.json({error:'Completed job not found.'},{status:404});
