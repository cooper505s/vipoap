...
export async function onRequestPatch({request,env}){
  ...
  const body=await request.json(),key=clean(body.key,240),booking=key.startsWith('booking:')?await env.VIPOAP_DATA.get(key,'json'):null;if(!booking||!canAccessTerritory(context,booking.territoryId||'andover')||!completedBooking(booking))return Response.json({error:'Completed job not found.'},{status:404});
