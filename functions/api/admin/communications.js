...
export async function onRequestPost({request,env}){
  ...
  let deliveryStatus='recorded',providerId='',key='';
  if(direction==='outbound'&&channel==='Email'){
    ...
    deliveryStatus='sent';providerId=result.providerMessageId||'';key=result.communicationKey||'';
  ...
  if(!(direction==='outbound'&&['Email','WhatsApp'].includes(channel))){
    key=await recordCommunication(env,{customerId,channel,direction,event,recipient:channel==='Email'?customer.email:customer.phone,subject,message,deliveryStatus,providerMessageId:providerId,metadata:{createdBy:context.email}});
  }
  await audit(env,context,'record','customer-communication',customerId,{customerId,channel,direction,deliveryStatus,event});
  return Response.json({ok:true,key,deliveryStatus,providerId});
}
