...
export async function onRequestGet({request,env}){
  ...
  const [bookings,receipts,recommendations,delegates]=await Promise.all([records(env.VIPOAP_DATA,'booking:'),records(env.VIPOAP_DATA,'invoice:'),records(env.VIPOAP_DATA,'equipment-recommendation:'),records(env.VIPOAP_DATA,'delegated-access:')]);
  ...
  return Response.json({customer:redacted,entitlement:await membershipEntitlement(env,session.customerId),appointments:bookings.filter(own).sort((a,b)=>`${b.date} ${b.time}`.localeCompare(`${a.date} ${a.time}`)),receipts:receipts.filter(own),equipment:recommendations.filter(own),delegatedAccess:delegates.filter(x=>x.customerId===session.customerId||x.delegateCustomerId===session.customerId).map(x=>({key:x.key,customerId:x.customerId,delegateCustomerId:x.delegateCustomerId,permissions:x.permissions,status:x.status}))});
}
