...
export async function onRequestGet({env}){
  if(!env.VIPOAP_DB)return json({error:'Service catalogue is temporarily unavailable.'},503);
  try{
    ...
    for(const service of services.results||[]){
      ...
      const item={
        id:service.id,slug:service.slug,name:service.public_name||service.name,description:service.description||'',
        fulfilmentTypes,defaultDurationMinutes:service.default_duration_minutes,
        minDurationMinutes:service.min_duration_minutes,maxDurationMinutes:service.max_duration_minutes,metadata
      };
      if(!byCategory.has(service.category_id))byCategory.set(service.category_id,[]);
      byCategory.get(service.category_id).push(item);
    }
    return json({categories:(categories.results||[]).map(category=>({
      id:category.id,slug:category.slug,name:category.public_name||category.name,description:category.description||'',
      requiresHomeVisit:Boolean(category.requires_home_visit),allowsRemote:Boolean(category.allows_remote),
      services:byCategory.get(category.id)||[]
    }))});
  }catch(error){
    console.error(JSON.stringify({event:'service_catalog_error',message:error instanceof Error?error.message:'Unknown error'}));
    return json({error:'Service catalogue is temporarily unavailable.'},503);
  }
}
