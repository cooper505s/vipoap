...
async function getSettings(env,supportType){if(!env.VIPOAP_DATA)return [{weekly:DEFAULT_AVAILABILITY,blockedDates:[]}];const keys=await env.VIPOAP_DATA.list({prefix:'availability:'}),records=(await Promise.all(keys.keys.map(key=>env.VIPOAP_DATA.get(key.name,'json')))).filter(record=>record?.territoryId==='andover');if(records.length)return records.map(record=>({weekly:supportType==='Remote support'?record.remoteWeekly:record.homeVisitWeekly,blockedDates:record.blockedDates||[],overrides:record.overrides||[]}));const saved=await env.VIPOAP_DATA.get('availability','json');return[saved||{weekly:DEFAULT_AVAILABILITY,blockedDates:[]}]}
