...
async function getSettings(env,supportType){if(!env.VIPOAP_DATA)return [{weekly:DEFAULT_AVAILABILITY,blockedDates:[]}];const service=supportType==='Remote support'?'remote':'home',keys=await env.VIPOAP_DATA.list({prefix:'availability:'}),records=(await Promise.all(keys.keys.map(key=>env.VIPOAP_DATA.get(key.name,'json')))).filter(record=>record?.territoryId==='andover'),eligible=[];for(const record of records){const operator=record.operatorId?await env.VIPOAP_DATA.get(`operator:${record.operatorId}`,'json'):null,services=Array.isArray(operator?.serviceTypes)?operator.serviceTypes:['home','remote'];if(services.includes(service))eligible.push(record)}if(eligible.length)return eligible.map(record=>({weekly:supportType==='Remote support'?record.remoteWeekly:record.homeVisitWeekly,blockedDates:record.blockedDates||[],overrides:record.overrides||[]}));if(records.length)return[];const saved=await env.VIPOAP_DATA.get('availability','json');return[saved||{weekly:DEFAULT_AVAILABILITY,blockedDates:[]}]}
