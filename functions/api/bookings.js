...
import {initialBookingState,validPaymentMethod} from '../_shared/booking-state.js';
...
export async function onRequestPost({request,env}){
  ...
  if(!validPaymentMethod(booking.supportType,booking.paymentMethod))return jsonError('Choose a valid payment method for this service.',400);
  ...
  const ref=reference(),slotKey=`booking:${booking.date}:${booking.time}`,now=new Date().toISOString(),initialState=initialBookingState(booking);
  await env.VIPOAP_DATA.put(slotKey,JSON.stringify({...booking,...initialState,reference:ref,status:'pending',adminNotes:'',createdAt:now,updatedAt:now}));
  ...
  await env.VIPOAP_DATA.put(slotKey,JSON.stringify({...booking,...initialState,reference:ref,status:'pending',adminNotes:'',customerId:customer?.key||'',territoryId:customer?.territoryId||'andover',operatorId:customer?.operatorId||'dan-stevens',createdAt:now,updatedAt:new Date().toISOString()}));
  ...
  return Response.json({ok:true,reference:ref,...initialState});
}
