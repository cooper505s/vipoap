...
export async function onRequestPost({request,env}){
  ...
  if(env.VIPOAP_DATA&&await env.VIPOAP_DATA.get(slotKey))return Response.json({error:'That appointment has just been taken. Please choose another time.'},{status:409});
  const ref=reference();
  if(env.VIPOAP_DATA)await env.VIPOAP_DATA.put(slotKey,JSON.stringify({...booking,reference:ref,createdAt:new Date().toISOString()}));
