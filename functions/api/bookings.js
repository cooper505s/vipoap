...
async function sendEmails(env,booking,ref){
  ...
  const adminHtml=`<h2>New VIPOAP booking request</h2><p><strong>Reference:</strong> ${escapeHtml(ref)}</p><p><strong>Name:</strong> ${safe.name}</p><p><strong>Telephone:</strong> ${safe.phone}</p><p><strong>Email:</strong> ${safe.email||'Not supplied'}</p><p><strong>Postcode:</strong> ${safe.postcode}</p><p><strong>Service:</strong> ${safe.service}</p><p><strong>Date:</strong> ${safe.date}</p><p><strong>Time:</strong> ${safe.time}</p><p><strong>Duration:</strong> ${safe.duration} minutes</p><p><strong>Details:</strong><br>${safe.details||'Not supplied'}</p>`;
