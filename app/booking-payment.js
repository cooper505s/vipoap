(function(){
  ...
  function priceFor(type,duration){if(type==='Remote support')return duration===30?'£15':'£25';return duration===30?'£39':'£64'}
  function update(){
    ...
    if(price30)price30.textContent=remote?'30 minutes — £15':'First 30 minutes — £39';
    if(price60)price60.textContent=remote?'Up to 60 minutes — £25':'Up to 1 hour — £64';
    if(priceNote)priceNote.innerHTML=remote?'<strong>Secure, customer-approved support.</strong><br>Further time is agreed first. We never unexpectedly call asking for remote access, and we never leave permanent unattended access.':'<strong>Simple home-visit pricing.</strong><br>£39 includes the visit and first 30 minutes. Each additional 30 minutes is £25 and is only charged with your agreement. Local travel is included.';
    document.getElementById('paymentHelp').innerHTML='<strong>'+support.value+' is paid securely before confirmation.</strong><br>Any additional time is agreed with you first and billed after the appointment.';
  }
